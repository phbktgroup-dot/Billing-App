...
export const generateInvoicePDF = async (invoice: InvoiceData, business: BusinessProfile, returnBlob: boolean = false) => {
  ...
  doc.text(`Vehicle No. : ${invoice.eway_data?.vehicleNo || ""}`, 108, 84);
...
export const generateEwayBillPDF = async (invoice: InvoiceData, business: BusinessProfile, returnBlob: boolean = false) => {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  const eway = invoice.eway_data;
  ...
  doc.setFontSize(18);
  doc.setFont("helvetica", "bold");
  doc.text("e-Way Bill", 12, 20);

  // Barcode Placeholder (Top Right)
  doc.rect(160, 10, 35, 35);
  doc.setFontSize(6);
  doc.text("QR CODE", 177.5, 27.5, { align: 'center' });

  // 1. E-WAY BILL Details
  doc.setFontSize(9);
  doc.setFillColor(245, 245, 245);
  doc.rect(10, 40, 190, 6, 'F');
  doc.rect(10, 40, 190, 6);
  doc.text("1. E-WAY BILL Details", 12, 44.5);

  doc.rect(10, 46, 190, 35);
  doc.setFont("helvetica", "bold");
  doc.text(`eWay Bill No: ${invoice.eway_bill_no || ""}`, 12, 52);
  doc.text(`Generated Date: ${eway.generatedDate || new Date().toLocaleString('en-GB')}`, 75, 52);
  doc.text(`Generated By: ${eway.generatedBy || business.gst_number || ""}`, 140, 52);
  
  doc.text(`Valid Upto: ${eway.validUpto || ""}`, 140, 58);

  doc.setFont("helvetica", "normal");
  const modeMap: Record<string, string> = { '1': 'Road', '2': 'Rail', '3': 'Air', '4': 'Ship' };
  doc.text(`Mode: ${modeMap[eway.transMode] || 'Road'}`, 12, 64);
  doc.text(`Approx Distance: ${eway.transDistance}km`, 75, 64);

  const subTypeMap: Record<string, string> = {
    '1': 'Supply', '2': 'Export', '3': 'Job Work', '4': 'SKD/CKD/Lots',
    '5': 'Recipient Not Known', '6': 'For Own Use', '7': 'Exhibition or Fairs', '8': 'Others'
  };
  doc.text(`Type: ${eway.supplyType === 'O' ? 'Outward' : 'Inward'} - ${subTypeMap[eway.subSupplyType] || 'Others'}`, 12, 72);
  
  const docTypeMap: Record<string, string> = { 'INV': 'Tax Invoice', 'BIL': 'Bill of Supply', 'BOE': 'Bill of Entry', 'OTH': 'Others' };
  doc.text(`Document Details: ${docTypeMap[eway.documentType] || 'Tax Invoice'} -`, 75, 72);
  doc.setFont("helvetica", "bold");
  doc.text(`${invoice.invoice_number} - ${new Date(invoice.date).toLocaleDateString('en-GB')}`, 115, 72);
  
  doc.setFont("helvetica", "normal");
  const transTypeMap: Record<number, string> = { 1: 'Regular', 2: 'Bill To - Ship To', 3: 'Bill From - Dispatch From', 4: 'Combination of 2 and 3' };
  doc.text(`Transaction type: ${transTypeMap[eway.transactionType] || 'Regular'}`, 140, 72);
  doc.text(`Portal: 1`, 175, 72);

  // 2. Address Details
  doc.setFillColor(245, 245, 245);
  doc.rect(10, 85, 190, 6, 'F');
  doc.rect(10, 85, 190, 6);
  doc.text("2. Address Details", 12, 89.5);

  doc.rect(10, 91, 95, 45); // From
  doc.rect(105, 91, 95, 45); // To

  doc.setFont("helvetica", "bold");
  doc.text("From", 12, 96);
  doc.setFont("helvetica", "normal");
  doc.text(`GSTIN : ${business.gst_number || ""}`, 12, 101);
  doc.text(business.name || "", 12, 106);
  doc.text(business.state || "", 12, 111);
  doc.text(":: Dispatch From ::", 12, 118);
  doc.text(`${business.address1 || ""}, ${business.address2 || ""}`, 12, 123);
  doc.text(`${business.city || ""}, ${business.state || ""} - ${business.pincode || ""}`, 12, 128);

  doc.setFont("helvetica", "bold");
  doc.text("To", 107, 96);
  doc.setFont("helvetica", "normal");
  doc.text(`GSTIN : ${invoice.customer_gstin || ""}`, 107, 101);
  doc.text(invoice.customer_name || "", 107, 106);
  doc.text(invoice.customer_state || "", 107, 111);
  doc.text(":: Ship To ::", 107, 118);
  const addrLines = (invoice.customer_address || "").split('\n');
  doc.text(addrLines[0] || "", 107, 123);
  doc.text(addrLines[1] || "", 107, 128);

  // 3. Goods Details
  doc.setFillColor(245, 245, 245);
  doc.rect(10, 140, 190, 6, 'F');
  doc.rect(10, 140, 190, 6);
  doc.text("3. Goods Details", 12, 144.5);

  autoTable(doc, {
    startY: 146,
    head: [['HSN Code', 'Product Name & Desc.', 'Quantity', 'Taxable Amount Rs.', 'Tax Rate (C+S+I+Cess)']],
    body: invoice.items.map(item => [
      item.hsnCode || "",
      item.name,
      item.quantity.toFixed(2),
      item.amount.toFixed(2),
      `${(item.gstRate || 0)/2}+${(item.gstRate || 0)/2}+0+0`
    ]),
    theme: 'grid',
  ...
  autoTable(doc, {
    startY: currentY,
    head: [['Tot. Tax\'ble Amt', 'CGST Amt', 'SGST Amt', 'IGST Amt', 'CESS Amt', 'Other Amt', 'Total Inv.Amt']],
    body: [[
      invoice.subtotal.toFixed(2),
      (invoice.cgst_amount || 0).toFixed(2),
      (invoice.sgst_amount || 0).toFixed(2),
      (invoice.igst_amount || 0).toFixed(2),
      (eway.TotNonAdvolVal || 0).toFixed(2),
      (eway.OthValue || 0).toFixed(2),
  ...
  doc.text("4. Transportation Details", 12, currentY + 4.5);
  currentY += 6;

  doc.rect(10, currentY, 190, 10);
  doc.text(`Transporter ID & Name : ${eway.transporterId || ""} & ${eway.transporterName || ""}`, 12, currentY + 6);
  doc.text(`Transporter Doc. No & Date : ${eway.transDocNo || ""} & ${eway.transDocDate || ""}`, 105, currentY + 6);
  currentY += 15;
  ...
  doc.rect(10, currentY, 190, 6, 'F');
  doc.rect(10, currentY, 190, 6);
  doc.text("5. Vehicle Details", 12, currentY + 4.5);
  currentY += 6;

  autoTable(doc, {
    startY: currentY,
    head: [['Mode', 'Vehicle / Trans Doc No & Dt.', 'From', 'Entered Date', 'Entered By', 'Portal']],
    body: [[
      eway.transMode === '1' ? 'Road' : 'Others',
      `${eway.vehicleNo || ""} & ${eway.transDocNo || ""} & ${eway.transDocDate || ""}`,
      business.city || "",
      eway.generatedDate || new Date().toLocaleString('en-GB'),
      business.gst_number || "",
      "1"
    ]],
    theme: 'grid',
  ...
  const barcodeX = 85;
  for(let i=0; i<40; i++) {
    const w = Math.random() > 0.5 ? 0.8 : 0.3;
    doc.setLineWidth(w);
    doc.line(barcodeX + (i * 1), currentY, barcodeX + (i * 1), currentY + 15);
  }
  doc.setFontSize(8);
  doc.text(invoice.eway_bill_no || "292173433204", 105, currentY + 20, { align: 'center' });

  if (returnBlob) {
    return doc.output('blob');
  } else {
    await downloadFile(doc.output('blob'), `EWayBill_${invoice.eway_bill_no || invoice.invoice_number}.pdf`);
  }
};
